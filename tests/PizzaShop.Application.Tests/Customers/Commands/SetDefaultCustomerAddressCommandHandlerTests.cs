...
public class SetDefaultCustomerAddressCommandHandlerTests
{
    ...
    private SetDefaultCustomerAddressCommandHandler CreateHandler() =>
        new(_customerRepository.Object, _unitOfWork.Object, _currentUser.Object);

    private static Customer CreateCustomerWithAddress(out CustomerAddress entry)
    {
        ...
        entry = customer.AddAddress("Dom", address);
        return customer;
    }

    [Fact]
    public async Task Handle_ExistingAddress_SetsItAsDefault()
    {
        var customerId = Guid.NewGuid();
        _currentUser.Setup(c => c.CustomerId).Returns(customerId);

        var customer = CreateCustomerWithAddress(out var entry);
        _customerRepository
        ...
        await handler.Handle(new SetDefaultCustomerAddressCommand(entry.Id), CancellationToken.None);

        customer.AddressBook.Single().IsDefault.Should().BeTrue();
        _customerRepository.Verify(r => r.UpdateAsync(customer, It.IsAny<CancellationToken>()), Times.Once);
        _unitOfWork.Verify(u => u.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_UnknownAddressId_ThrowsAddressNotInAddressBookException()
    {
        var customerId = Guid.NewGuid();
        _currentUser.Setup(c => c.CustomerId).Returns(customerId);

        var customer = CreateCustomerWithAddress(out _);
        _customerRepository
            .Setup(r => r.GetByIdAsync(customerId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(customer);
        ...
        var act = () => handler.Handle(new SetDefaultCustomerAddressCommand(Guid.NewGuid()), CancellationToken.None);

        await act.Should().ThrowAsync<AddressNotInAddressBookException>();
    }
