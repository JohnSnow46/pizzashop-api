...
public class MarkReadyCommandHandlerTests
{
    private readonly Mock<IOrderRepository> _orderRepository = new();
    private readonly Mock<IOrderNotifier> _orderNotifier = new();
    private readonly Mock<IEmailSender> _emailSender = new();
    private readonly Mock<IUnitOfWork> _unitOfWork = new();
    private readonly Mock<ILogger<MarkReadyCommandHandler>> _logger = new();

    private MarkReadyCommandHandler CreateHandler() =>
        new(_orderRepository.Object, _orderNotifier.Object, _emailSender.Object, _unitOfWork.Object, _logger.Object);

    [Fact]
    public async Task Handle_OrderInPreparation_MarksReadyAndNotifies()
    {
        ...
        await handler.Handle(new MarkReadyCommand(order.Id), CancellationToken.None);
        ...
        _orderNotifier.Verify(
            n => n.OrderStatusChangedAsync(order.Id, OrderStatus.Ready, order.EstimatedReadyAt, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task Handle_UnknownOrder_ThrowsNotFoundException()
    {
        ...
        var act = () => handler.Handle(new MarkReadyCommand(Guid.NewGuid()), CancellationToken.None);
