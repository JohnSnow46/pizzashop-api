...
public class StartPreparationCommandHandlerTests
{
    ...
    private StartPreparationCommandHandler CreateHandler() =>
        new(_orderRepository.Object, _orderNotifier.Object, _unitOfWork.Object);

    [Fact]
    public async Task Handle_AcceptedOrder_StartsPreparationAndNotifies()
    {
        ...
        await handler.Handle(new StartPreparationCommand(order.Id), CancellationToken.None);
        ...
        _orderNotifier.Verify(
            n => n.OrderStatusChangedAsync(order.Id, OrderStatus.InPreparation, order.EstimatedReadyAt, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task Handle_UnknownOrder_ThrowsNotFoundException()
    {
        ...
        var act = () => handler.Handle(new StartPreparationCommand(Guid.NewGuid()), CancellationToken.None);
